...
class HumanoidIKEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None, initial_pose=None):
        ...
        # Disconnect if already connected, otherwise p.connect() might fail
        if self._physics_client_id >= 0 and p.isConnected(self._physics_client_id):
        ...
        start_pos = [0, 0, 3.4]  # This height MUST match the converter's offset
        ...
        if initial_pose is not None:
            # Note: The converter now sends "geometric" angles, not "IK" angles
            print("  [Env Reset] Applying initial pose from Geometric Converter...")
            if len(initial_pose) != len(self.controllable_joint_indices):
                ...
                for i, joint_index in enumerate(target_indices):
                    if 0 <= joint_index < self.num_total_joints:
                        ...
                        # Get joint info to check its type
                        info = p.getJointInfo(
                            self.robot_id,
                            joint_index,
                            physicsClientId=self._physics_client_id
                        ...
                        joint_type = info[2]
                        angle = target_poses[i]  # This is our target angle (float)

                        if joint_type == p.JOINT_REVOLUTE or joint_type == p.JOINT_PRISMATIC:
                            # This is a simple 1-DOF joint, use resetJointState
                            p.resetJointState(
                            ...
                            # This is a complex 3-DOF joint, requires a quaternion
                            # and the MultiDof function.
                            # We assume rotation is around the X-axis (1,0,0) as per the URDF warning
                            quat_angle = p.getQuaternionFromEuler([0, 0, angle])

                            p.resetJointStateMultiDof(
                                bodyUniqueId=self.robot_id,
                                jointIndex=joint_index,
                                targetValue=quat_angle,  # Pass the quaternion
                                targetVelocity=[0, 0, 0],  # Must be a 3D vector
                                physicsClientId=self._physics_client_id
                            )
                        # --- END OF FINAL FIX ---
                    else:
                ...
                # Step simulation a few times to let the pose settle
    ...
    def close(self):
        ...
        # (P1) CRASH FIX
        # ========================================================================
        # Check if we are still connected before trying to disconnect
        if self._physics_client_id >= 0 and p.isConnected(self._physics_client_id):
            p.disconnect(self._physics_client_id)
